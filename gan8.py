...
# Define the Generator and Discriminator with deeper layers
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        lstm_out = nn.functional.relu(self.fc1(lstm_out))
        lstm_out = nn.functional.relu(self.fc2(lstm_out))
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        lstm_out, _ = self.lstm(x)
        lstm_out = nn.functional.relu(self.fc1(lstm_out[:, -1, :]))
        lstm_out = nn.functional.relu(self.fc2(lstm_out))
        out = self.fc3(lstm_out)
        out = self.batch_norm(out)
...
# Hyperparameters
hidden_dim = 128  # Increased number of units
...
for epoch in range(num_epochs):
    ...
    torch.save(netG.state_dict(), "/Volumes/NO NAME/ABEL-body-motion/gan8_models/generator8.pth")
    torch.save(netD.state_dict(), "/Volumes/NO NAME/ABEL-body-motion/gan8_models/discriminator8.pth")
