...
def normalize_df(df):
    min_vals = df.min()
    max_vals = df.max()
    print("df min: ", min_vals, "\tdf max: ", max_vals)
    denominator = max_vals - min_vals
    denominator[denominator == 0] = 1  # Avoid division by zero
    normalized_df = (df - min_vals) / denominator
    return normalized_df, min_vals, max_vals

train_df, train_min, train_max = normalize_df(train_df)
val_df, val_min, val_max = normalize_df(val_df)
test_df, test_min, test_max = normalize_df(test_df)

# Function to save min and max values to a JSON file
def save_min_max(min_vals, max_vals, filename):
    min_max = {
...
save_min_max(train_min, train_max, 'train_min_max.json')
...
# Apply PCA
def apply_pca(train, val, test, n_components, filename):
...
train_speech_pca, val_speech_pca, test_speech_pca = apply_pca(train_speech, val_speech, test_speech, n_components_speech, 'pca_speech_model.pkl')
train_joint_pca, val_joint_pca, test_joint_pca = apply_pca(train_joint, val_joint, test_joint, n_components_joint, 'pca_joint_model.pkl')
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        epochs_no_improve = 0
        torch.save(model.state_dict(), f"/Volumes/NO NAME/ABEL-body-motion/tgan2_models/best_model2.pth")
    else:
        epochs_no_improve += 1
...
print(f'Test Loss: {test_loss:.4f}, Test MAE: {test_mae:.4f}, Test RMSE: {test_rmse:.4f}, Test R2: {test_r2:.4f}')
