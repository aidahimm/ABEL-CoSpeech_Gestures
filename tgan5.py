...
hidden_dim = 256
num_layers = 2  # Revert to the original number of layers
...
for epoch in range(num_epochs):
    ...
    metrics['validation_mae'].append(val_mae)
    metrics['validation_rmse'].append(val_rmse)
    metrics['validation_r2'].append(val_r2)
    ...
    if val_loss < best_val_loss:
        ...
        epochs_no_improve = 0
        torch.save(model.state_dict(), f"/Volumes/NO NAME/ABEL-body-motion/tgan5_models/best_model.pth")
    else:
        epochs_no_improve += 1
...
metrics_df.to_csv('/Volumes/NO NAME/ABEL-body-motion/tgan5_models/metrics.csv', index=False)
